...
class GraphWaveNet(nn.Module):
    ...
    def __init__(self, num_nodes, in_dim=3, out_dim=1, horizon=12, supports=None, adp_adj=True, 
                 ...
                 end_channels=512, kernel_size=2, blocks=4, layers=2, use_attn=True, emb_dim=10):
        ...
        if self.adp_adj:
            self.nodevec1 = nn.Parameter(torch.randn(num_nodes, emb_dim), requires_grad=True)
            self.nodevec2 = nn.Parameter(torch.randn(emb_dim, num_nodes), requires_grad=True)
        ...
        self.end_conv_2 = nn.Conv2d(end_channels, horizon * out_dim, kernel_size=(1, 1))
...
if __name__ == "__main__":
    print("Testing SOTA PyTorch 2.x GWNet Architecture across Dynamic Node Dimensions...")
    for test_nodes in [207, 716]:
        model = GraphWaveNet(num_nodes=test_nodes, in_dim=3, out_dim=1, horizon=12)
        dummy_x = torch.randn(2, 3, test_nodes, 13)
        out = model(dummy_x)
        print(f"  [+] GWNet Forward Pass Success ({test_nodes} nodes): Input {dummy_x.shape} -> Output {out.shape}")
